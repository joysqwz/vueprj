...
class UserService {
	// async registration(email, password, ipAddress, userAgent) {
	// 	try {
	...
	// 			)
	// 			logger.info(`Пользователь добавлен в таблицу users: ${email}`)
	// 			if (role === 'lecturer') {
	// 				await pool.query(
	...
	//                        VALUES ($1, $2, $3, $4, $5)`,
	// 					[userId, first_name, middle_name, last_name, group_id || null]
	// 				)
	// 			}
	// 			const tokens = tokenService.generateTokens({ id: userId, role })
	// 			// УКАЗАТЬ СВОЙ UNIQUE_ID >>>>>>
	// 			await tokenService.saveToken('0af5161f-cb5f-43fc-87a2-e6847d3107c3', userId, tokens.refreshToken, ipAddress, userAgent, tokens.jti)
	...
	async quitSessions(userId) {
		try {
			...
			await pool.query('BEGIN')
			await pool.query(
				`UPDATE token SET is_revoked = TRUE WHERE user_id = $1`,
				[userId]
			)
			await pool.query('COMMIT')
